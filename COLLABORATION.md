...
- **专用账号**：Windows 录制机使用专用知乎账号，`zhihu_auth_state.json` 由该账号登录生成。
- **单设备原则**：直播录制期间，该账号不在任何其他设备或浏览器登录知乎直播间，避免 CC/知乎风控触发会话被踢。
- **Cookie 预检**：`run_zhihu_live.bat` 启动前自动运行 `scripts/check_auth.py`，检测 `z_c0` 缺失/空值/过期三种情况，任一失败则提示重新登录后退出。
- **失效处理**：代码层检测到页面跳转至登录页时打印 `[账号态失效]` 日志；无法阻止知乎服务端主动踢下线，只能重新登录后继续。
