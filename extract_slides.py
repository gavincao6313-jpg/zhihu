...
def _load_stream_slide_frames(base_stem: str) -> list[Path]:
    ...
    for pf in payload_files:
        ...
        if not m:
            continue
        key = m.group(1)
        if key not in chunk_map or pf.stat().st_mtime > chunk_map[key].stat().st_mtime:
            chunk_map[key] = pf

    def _offset(p: Path) -> int:
        m = re.search(r"_chunk\d+_(\d+)s", p.stem)
        return int(m.group(1)) if m else 0

    sorted_payloads = sorted(chunk_map.values(), key=_offset)

    slide_frames: list[Path] = []
    for pf in sorted_payloads:
        try:
...
def extract_slides(
    ...
    # ── Step 3: 输出 PDF + PPTX ────────────────────────────────────────────────
    _output_slides(final_frames, out_dir, stem)

    return out_dir


def extract_slides_stream(
    base_stem: str,
    sim_threshold: float = 8.0,
    dedup: bool = True,
) -> Path | None:
    ...
    Reads per-chunk payload.json from runs/, collects existing slide-type JPEGs
    ...
    # ── Dedup ──
    if dedup:
        deduped = _deduplicate(working_frames, sim_threshold)
        removed = len(working_frames) - len(deduped)
        if removed:
            print(f"  [{base_stem}] 去重: 移除 {removed} 张相似帧，保留 {len(deduped)} 张")
    else:
        deduped = working_frames
    ...
    _output_slides(final_frames, out_dir, base_stem)
    return out_dir
